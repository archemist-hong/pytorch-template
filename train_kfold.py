...
for k, (train_indices, valid_indices) in enumerate(skf.split(range(len(dataset)), dataset.labels)):
    print(f"\n fold: {k} \n")
    # check directories
    check_directories(config.get('training')['experiment_path']+f"-{k}")
    ...
    if config.get('tensorboard'):
        ...
        writer = SummaryWriter(config.get('training')['experiment_path']+f"-{k}")
            ...
            if (phase == "valid"):
                if epoch_loss < best_loss:
                    ...
                    torch.save({
                        ...
                        os.path.join(config.get('training')['experiment_path']+f"-{k}", 'checkpoint', f"checkpoint_model_{epoch}_{epoch_loss:.3f}_{epoch_final_f1:.3f}.pt"))# save checkpoint
            if slack_enable:
    ...
    pd.DataFrame(mask_result.numpy()).to_csv(os.path.join(config.get('training')['experiment_path']+f"-{k}", 'mask_cm.csv'))
    pd.DataFrame(gender_result.numpy()).to_csv(os.path.join(config.get('training')['experiment_path']+f"-{k}", 'gender_cm.csv'))
    pd.DataFrame(age_result.numpy()).to_csv(os.path.join(config.get('training')['experiment_path']+f"-{k}", 'age_cm.csv'))
    pd.DataFrame(final_result.numpy()).to_csv(os.path.join(config.get('training')['experiment_path']+f"-{k}", 'final_cm.csv'))